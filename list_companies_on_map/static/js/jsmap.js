        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (const comp of Object.values(companies)) {
                ...
                object.then(function (res) {
                    ...
                    myMap.geoObjects.add(new ymaps.Placemark([coor[1], coor[0]], {balloonContent: `Компания: <strong>${comp['title']}</strong>` + '\n' + `Адрес: ${comp['addr'][0]['ADDRESS_1']}`}, ));
                })
            }
        }
    };

    req.open("GET", "/list_companies_on_map/all_companies");
    req.send();
}