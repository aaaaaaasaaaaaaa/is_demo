...
def find_finish_task(request):
    ...
    if request.method == "POST":
        ...
        app_tasks_id = get_app_tasks_id(but) # Достаем ID задач из опций приложения
        if not app_tasks_id or app_tasks_id[0] == '':
            return render(request, 'best_call_manager_temp.html')

        app_tasks = get_app_tasks(but, app_tasks_id) # Фильтруем по ID и достаем информацию по этим задачам из Б24

        progress_tasks_id = app_tasks_id if app_tasks_id else [] # ID незавершенных задач
        completed_tasks = dict() # Информация о завершенных
        ...
        possible_calls = but.call_api_method("app.option.get", {"option": "possible_calls"})["result"] # Список подходящих звонков
        ...
        set_app_tasks_id(but, progress_tasks_id) # Обновляем список ID задач
        but.call_api_method("app.option.set", {"options": {"possible_calls": possible_calls}})


        # Создбфаем html-таблицу
