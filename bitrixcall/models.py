...
class MyNumberChoicesType(models.IntegerChoices):
    '''Тип звонка'''
...
class MyNumberChoicesAddToChat(models.IntegerChoices):
    '''Уведомление сотрудника Б24'''
...
class MyCallInfo(models.Model):
    '''Модель хранящая данные звонков'''
    ...
    type = models.IntegerField(null=False, blank=False,
                               choices=MyNumberChoicesType.choices)
    duration = models.IntegerField(null=True, blank=True)
    add_to_chat = models.IntegerField(blank=True, null=True,
                                      choices=MyNumberChoicesAddToChat.choices)
    ...
    def telephony_externalcall_register(self, but):
        '''Метод регистрации звонка'''

        res = but.call_api_method("telephony.externalcall.register", { # получает параметры звонка
    ...
    def telephony_externalcall_finish(self, but):
        '''Метод завершения звонка'''

        but.call_api_method('telephony.externalcall.finish', {
            "CALL_ID": self.call_id,
            "USER_ID": self.user_id,
            "DURATION": self.duration,
           # "RECORD_URL": f'https://{settings.APP_SETTINGS.app_domain}/media/{self.inner_media_path}{self.filename}.mp3', устаревший параметр
