...
def send_calls(but, bot, calls_chat_id, first_call_id):
    ...
    calls_with_files, len_calls = get_calls_with_files(but, first_call_id) # Получаем звонки с файлами и кол-во звонков
    # ID звонков начинаются с "2" и считаются через 2
    last_call_id = str(len_calls * 2 + int(first_call_id))
    if not calls_with_files:
        return None

    # Достаем все файлы по их ID
    methods = []
    ...
    for call_file in call_files.successes.values():
        call = calls_with_files[int(call_file['result']['ID'])]  # для каждого выбираем звонок с таким же ID файла
...
def export_calls_to_telegram(but, bot_token, calls_chat_id):
    ...
    last_call_id = send_calls(but, bot, calls_chat_id, first_call_id)
    # обновляем переменную опций приложения с ID последнего просмотренного звонка.
    if last_call_id is not None:
        but.call_api_method('app.option.set', {'options': {'last_call_id': last_call_id}})
        return True
    else:
